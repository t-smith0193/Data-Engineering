...
On top of that, there’s a layer of event-driven AWS Lambda functions that react to new data in real time. These handle things like alerting, Slack updates, anomaly detection, and triggering downstream analysis. There’s also an OpenAI-powered component that takes National Weather Service (NWS) forecast discussions (AFDs), filters them down to KSFO-relevant signals, and turns them into structured forecasts mapped directly to Kalshi market buckets.
